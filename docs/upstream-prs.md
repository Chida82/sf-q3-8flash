...
| #1056 | Metal: optimize Qwen3.8 kernels, MTP state and SSD MoE scheduling | open | `b1af94b` | 30 | 2026-09-23 | 2026-09-24 | `40` took the HC reuse only (plain decode +0.8%; Q8 commits measured and dropped), MTP ideas -> `30`, `13c53d9`'s shared Q8 loop -> `50` (plain decode +1.3%), SSD -> `80`, IQ2/Q2_K -> `70` (its IQ2 gate/up and Q2_K down decode kernels on M5: Q2 decode +9.0%, MTP decode +8.1-8.7% against main, from the two kernels and the specialization they superseded; Q2 tails measured and dropped by the MTP-decode guard), `496b153`'s tile widths -> `60` (bit-identical remainder tails); chunk-invariant prefill dropped |
...
| `cdfc0d5` | Metal: reuse Qwen IQ2 and HC inputs on M1 Max | adopt -> `40-dense-decode-kernels` (HC part); adopt -> `70-q2-kernels-m5` (IQ2 part) | `40` took the HC gate-mix reuse kernel for one decode token on M5 (F16, rank 320): byte-exact, plain decode +0.8% (12 pairs, bootstrap 95% CI +0.3..+1.5%), MTP unchanged. Not taken: the M1 Max gate, T >= 3 selection, the M1 benchmark scaffolding. `70` took the IQ2 part as `kernel_qwen4_moe_mid_iq2` (NR1 for one or two tokens, NR2 above) on M5, without the M1 gate and SSD slot plumbing: byte-exact against the generic kernel, Q2 decode moe_mid GPU time -17.9% after the specialization of the step before, Q2 plain decode +1.6%, MTP decode +2.1-2.6% |
| `496b153` | Metal: adapt Qwen SSD MoE tiles and specialize low-bit prefill | idea -> `60-q4-expert-prefill` | `60` took tile width by tokens per expert as 16- and 8-token tails for each expert's remainder on the half tiles: bit-identical; routed expert GPU time at about 540 tokens moe_mid -1.2%, moe_down -4.9%, about 0 at 2050 tokens, an end-to-end effect below what the A/B harness resolves (most of `60`'s prefill gain comes from its Q4_K staging loads); `61` measured a 48-token tile for remainders 33..48 on GPU section time and dropped it (routed MoE time +1.1% at 8192, +3.3% at +2048, +5.1% at +512: the extra tail dispatch costs more than the narrower tile saves); `70` measured its low-bit M5 counterpart, remainder tails for IQ2_XXS/Q2_K, three times: bit-identical, Q2 prefill +2..+3% at 8192, +19..+22% at +512, +6..+8% at +2048, MTP prefill +8..+11% (code) and +5..+8% (prose), but every campaign read one decode metric wholly below zero (MTP-code decode -0.53% on the P1 tree, plain decode -0.77% on the final tree; a variant limited to batches under 4096 tokens read plain decode -1.1%, MTP decode -0.3%), while the same tails off in both builds and two same-binary runs with only prefill speed changed read 0; the tails cannot run in decode (the tiled path needs more than 64 tokens) and decode GPU time per pass was not slower, so the cost is in wall-clock decode after a denser prefill, about -0.3% on average, and the step was dropped; the SSD sparse-tile part stays with `80` |
...
| `5cbdb6e` | Metal: pipeline Qwen SSD MoE prefill and reuse Q2_K down rows | adopt -> `80-qwen-ssd-streaming` | phase B; `70` took the Q2_K down-row reuse (with `9a8462a`) as `kernel_qwen4_moe_down_q2k` at width 640 on M5 |
...
| `a37fd7f` | Metal: reuse IQ2 headers when staging MoE prefill tiles | drop | the M5 tensor path (`qwen4_load_raw16`) already reads the IQ2 header once per 16 values; the `stage16` branch it changes runs only with `DS4_QWEN4_MOE_MM_NAX=0` |
| `13c53d9` | Metal: specialize Qwen IQ2 decode and reuse shared Q8 gate/up inputs | adopt -> `50-q4-expert-decode` (shared Q8 loop); adopt -> `70-q2-kernels-m5` (IQ2 part) | `50` took the shared Q8 gate/up loop as `qwen4_moe_shared_q8_mid` in the Q4_K per-token kernel: byte-exact, MoE mid -5.8% GPU time, plain decode +1.3% (14 pairs, 95% CI +1.0..+1.7%). `70` took the IQ2 part (see `cdfc0d5`), whose shared Q8 slot calls the same helper |
| `9a8462a` | Metal: streamline Qwen Q2 down loads and shared expert row reuse | adopt -> `70-q2-kernels-m5` | `70` took its shared Q8 row reuse as `qwen4_moe_shared_q8_rows`, first in the Q4 MXFP4 down kernel (Q4 decode moe_down GPU time -2.5%, bit-identical), then with the packed loads and tail-free blocks in `kernel_qwen4_moe_down_q2k` (Q2 decode moe_down -2.0%, plain decode +0.3%, MTP decode +1.2%) |
...
| `11811b9` | perf(metal): reuse half operands for large Qwen SSD MoE prefill | drop | already in the M5 tensor path: level 2 converts x to half once and the mid epilogue writes the half copy the down tiles read |
