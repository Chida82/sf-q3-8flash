...
at least four resident sessions for the eviction section:
    ...
    ./sf-q3-8flash-server -m qwen3.8-flash-next.gguf \
    ...
    python3 tests/test_server_vision_live_state.py \
        --section eviction --log /tmp/ds4-live-state.log
...
# sf-ablate(glm): the glm53 fixtures are gone; the assertions below are about
# slot routing and status codes, not image content, so any two distinct
# images work.  Repointed to this child's Qwen vision fixtures.
...
def tool_409_section(args):
    ...
    image = image_part("maple.png", "anthropic")
